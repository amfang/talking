...
var TTSModule = (function() {
  ...
  function textToSpeech() {
    ...
    Api.setResponsePayload = function(newPayloadStr) {
      ...
      //alert("text--payload: "+newPayloadStr);
      //newPayloadStr -> JSON then for playCurrentAudio -- different with car-dashboard
      var payload = JSON.parse(newPayloadStr);
      //alert("text--payload.output: "+payload.output);
  ...
  function playCurrentAudio(payload) {
    //alert("payload.stringfy: "+JSON.stringify(payload));
    //alert("payload.text: "+payload.text);
    fetch('/api/text-to-speech/token') // Retrieve TTS token -- app.js defined -> speech/text-to-speech server
      ...
      if (button.value === 'ON') {
        ...
        if (payload.text) { // If payload.text is defined
          ...
          //alert("audio1: "+audio+"---token: "+token);
